...
　 　本题中 URL 映射功能的配置由若干条 URL 映射规则组成。当一个请求到达时，URL 映射功能会将请求中的 URL 地址按照配置的先后顺序逐一与这些规则进行匹配。当遇到第一条完全匹配的规则时，匹配成功，得到匹配的规则以及匹配的参数。若不能匹配任何一条规则，则匹 配失败。
...
const string s_path = "([0-9A-Za-z-_\./]+)";/** 这个包含了前面两个正则，所以需要最后比较！！  */

string rule[N], na[N];//记录规则，对应的名字
...
int main()
{
    ...
    for (int i = 1; i <= n; i++) {
        string s1;
        cin >> s1 >> na[i];
        rule[i] = _deal(s1, i);///   /articles/<int>/ year_archive
    }
    for (int i = 1; i <= m; i++) {
        string str; cin >> str;
        string ans = "404";
        ...
        if (ans != "404")
            for (int j = 0; j < p[k].size(); j++) {
                if (p[k][j] == 1)     cout << " " <<stoi(sm[j+1]);// string to int
                else                  cout << " " << sm[j+1];
        }
        cout << "\n";
    }
